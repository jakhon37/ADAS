...
* **Its own range rate.** It runs an alpha-beta filter (:class:`_AlphaBetaRange`)
  ...
  backstop is not blind on the frame a hazard appears.  The seed is bounded by the
  ego speed and comes from the ego state, not from the tracker.
...
* **A second range channel when one exists.** ``TrackedObject.range_estimate`` and
  ...
  the two channels AGREE within ``range_disagreement_frac`` they are blended by
  ...
  whether the cross-check actually ran.
...
class _AlphaBetaRange:
    ...
    def update(
        ...
        if state is None or dt_s <= 1e-4 or not math.isfinite(measurement_m):
            self._states[key] = _RangeState(distance_m=measurement_m, rate_mps=seed, updates=1)
...
class SafetyArbiter:
    ...
    def arbitrate(
        ...
        lead = None
        if perception.ok:
            lead = self._assess_lead(ctx, ego_speed_mps, dt_s, mitigated)
        self.last_lead = lead
        ...
        if plan is None:
            ...
            self._check_plan(plan, ego_speed_mps, ego_ok, faults, mitigated)
        self._check_kinematics(ego_speed_mps, ego_ok, dt_s, aeb or hazard, mitigated)

        self._check_lane_departure(ctx, mitigated)
        steering_out = self._limit_steering(cmd_in.steering, ego_speed_mps, dt_s, mitigated)

        violations = faults + mitigated + hazards
        ...
        out = self._synthesise_command(
            cmd_in, steering_out, new_state, aeb, hazard, lead, dt_s, cmd_ok, ego_speed_mps
        )
    ...
    def _fuse_range(
        self, ctx: SafetyContext, obj: TrackedObject, mitigated: List[str]
    ) -> Tuple[float, RangeSource, bool]:
        """Combine the pinhole range with an independent channel if one exists.

        Returns ``(distance_m, source, disagreement)``.

        Behaviour, in the order the code tests it:

        1. **No usable second channel** -- absent, ``UNAVAILABLE``, non-finite, or
           confidence below ``min_range_confidence``.  The pinhole range is used and
           the source stays ``PINHOLE``, so a consumer can tell the cross-check did
           not run.  A channel that is only guessing is not a second opinion, and a
           low-confidence estimate is discarded rather than down-weighted.  This is
           also the path taken when the depth channel is switched off.
        2. **Agreement** within ``range_disagreement_frac`` -- confidence-weighted
           blend, then the nearer of (blend, pinhole, depth).  Under-reading range
           only ever brakes more.
        3. **Disagreement, second channel NEARER** -- the nearer value is adopted
           only after ``range_corroboration_frames`` consecutive disagreeing frames
           on this track.  Until then the PINHOLE range is used and the frame is
           reported as an uncorroborated disagreement.  A single-frame depth outlier
           at highway speed used to produce a full-authority emergency stop, which
           is a collision hazard of its own.
        4. **Disagreement, second channel FARTHER** -- never adopted.  Raising the
           range on the word of a disagreeing channel would reduce braking.

        A disagreement is a MITIGATED finding, not a health fault: two range
        channels differing by more than 30% for two seconds is the normal case for a
        monocular depth model, and it must not latch DISENGAGE.
        """
        lim = self.limits
        pinhole = obj.distance_m
        estimate: RangeEstimate | None = None
        if ctx.independent_ranges:
            estimate = ctx.independent_ranges.get(obj.track_id)
        if estimate is None:
        ...
        if (
            ...
            self._disagreement_streak.pop(obj.track_id, None)
            return pinhole, RangeSource.PINHOLE, False
        if estimate.confidence < lim.min_range_confidence:
            self._disagreement_streak.pop(obj.track_id, None)
            mitigated.append(
                "range_channel_low_confidence_%.2f_track%d"
                % (estimate.confidence, obj.track_id)
            )
            return pinhole, RangeSource.PINHOLE, False
        ...
        if relative > lim.range_disagreement_frac:
            streak = self._disagreement_streak.get(obj.track_id, 0) + 1
            self._disagreement_streak[obj.track_id] = streak
            if estimate.distance_m >= pinhole:
                mitigated.append(
                    "range_channel_disagreement_%.0f%%_farther_track%d"
                    % (relative * 100.0, obj.track_id)
                )
            ...
            if streak < lim.range_corroboration_frames:
                mitigated.append(
                    ...
                    % (relative * 100.0, streak, lim.range_corroboration_frames, obj.track_id)
                )
                return pinhole, RangeSource.PINHOLE, True
            mitigated.append(
                "range_channel_disagreement_%.0f%%_track%d" % (relative * 100.0, obj.track_id)
            )
            return estimate.distance_m, RangeSource.FUSED, True

        self._disagreement_streak.pop(obj.track_id, None)
        weight = _clamp(estimate.confidence, 0.0, 1.0)
        fused = (1.0 - weight) * pinhole + weight * estimate.distance_m
        return min(fused, pinhole, estimate.distance_m), RangeSource.FUSED, False
    ...
    def _assess_lead(
        ...
        A track the arbiter's filter has not seen before -- a new object, a track-id
        change, an occlusion exit, a range-jump re-init -- has its rate SEEDED at
        ``-ego_speed``: the safe prior that an unknown object is stationary in the
        world.  Seeding at zero, as the previous version did, made the backstop
        report ``ttc=inf`` and command no brake on the first frame a stationary car
        appeared 15 m ahead, and made the RSS minimum gap collapse (13.7 m instead
        of 27.8 m at 15 m/s) because ``lead_speed = ego_speed + 0`` is the
        ...
        alive_set = set(alive)
        for key in [k for k in self._disagreement_streak if k not in alive_set]:
            del self._disagreement_streak[key]
        for key in [k for k in self._range_source if k not in alive_set]:
            del self._range_source[key]
        ...
        for obj in ctx.tracks:
            ...
            distance_m, source, disagreement = self._fuse_range(ctx, obj, mitigated)

            # Switching range channel is a discontinuity in the SIGNAL, not motion
            # of the object. Differencing across it manufactures a rate: adopting a
            # corroborated 40 m depth reading against a 60 m pinhole produced
            # -45 m/s of "closing" for an ego doing 15 m/s behind a stationary car,
            # and that fired the AEB. Re-seed instead, exactly as for a range jump.
            previous_source = self._range_source.get(obj.track_id)
            if previous_source is not None and previous_source is not source:
                self._range_filter.forget(obj.track_id)
                mitigated.append(
                    "range_source_switch_%s_to_%s_track%d"
                    % (previous_source.value, source.value, obj.track_id)
                )
            self._range_source[obj.track_id] = source

            filtered_m, rate_mps, reinitialised = self._range_filter.update(
                obj.track_id, distance_m, dt_s, seed_rate_mps=seed_rate_mps
            )
            if reinitialised:
            ...
            lead_speed_mps = max(0.0, ego_speed_mps + rate_mps)
            rss = (
            ...
            rss = max(lim.absolute_min_gap_m, rss)
            ...
            candidate = LeadAssessment(
                ...
                coasting=obj.time_since_update > 0,
    ...
    def _classify_hazard(
        ...
        DISENGAGE. ``aeb`` forces MIN_RISK_MANEUVER, ``hazard`` forces LIMITED.
        """
        if lead is None:
            return False, False
        lim = self.limits
    ...
    def _check_plan(
        ...
        if ego_ok and math.isfinite(plan.target_speed_mps):
            ...
            if requested > lim.max_acceleration_mps2:
                mitigated.append(
                    "plan_accel_%.2f_above_%.2f" % (requested, lim.max_acceleration_mps2)
                )
    ...
    def _check_kinematics(
        ...
        if -accel > lim.max_deceleration_mps2 + 1e-6 and not hazard_active:
            mitigated.append(
                "measured_decel_%.2f_above_%.2f_unjustified" % (-accel, lim.max_deceleration_mps2)
            )
        if self._prev_accel_mps2 is not None:
            jerk = abs(accel - self._prev_accel_mps2) / dt_s
            limit = lim.max_jerk_emergency_mps3 if hazard_active else lim.max_jerk_mps3
            if jerk > limit + 1e-6:
    ...
    def _limit_steering(
        self, steering: float, ego_speed_mps: float, dt_s: float, mitigated: List[str]
    ) -> float:
    ...
    def _required_decel(
        ...
        if state is SafetyState.MIN_RISK_MANEUVER or state is SafetyState.DISENGAGE:
            ...
            required = max(required, min(demanded, lim.comfort_decel_mps2))
    ...
    def _synthesise_command(
        ...
        required = self._required_decel(state, aeb, hazard, lead)
        if required > 0.0:
            throttle = 0.0
        ...
        if lead is not None:
            parts.append(
                ...
                % (
                    ...
                    lead.distance_m,
                    lead.range_rate_mps,
                    "(seeded)" if lead.rate_updates <= 1 else "",
